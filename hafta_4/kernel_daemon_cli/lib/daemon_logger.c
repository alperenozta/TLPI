...
void log_level(log_level_t level, const char *format, ...)
{
    if (level > current_log_level) {
        return;  // o anki seviyeden daha ayrıntılıysa yazma
    }

    va_list argList;
    va_start(argList, format);

    // syslog’a uygun seviye map edelim
    int pri = LOG_INFO;
    switch (level) {
        case LOG_LEVEL_ERROR:   pri = LOG_ERR;     break;
        case LOG_LEVEL_WARNING: pri = LOG_WARNING; break;
        case LOG_LEVEL_INFO:    pri = LOG_INFO;    break;
        case LOG_LEVEL_DEBUG:   pri = LOG_DEBUG;   break;
    }

    vsyslog(pri, format, argList);   // syslog’a yaz
    va_end(argList);
}
